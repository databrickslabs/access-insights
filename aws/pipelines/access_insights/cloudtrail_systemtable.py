...
credentials_schema = StructType([StructField("accessKeyId", StringType(), True)])
ingest_path = spark.conf.get("ingestPath")
start_date = spark.conf.get("startDate")
processed_date = F.now()
sourced_from_cloudlogs = 1 if ingest_path is not None and ingest_path != "" else 0

if ingest_path != None:

    @dlt.view
    def cloudtrail_logs_with_storagepath():
        json_data = dlt.read("access_insights.system_logs.cloudtrail_logs").select(
            "*",
            get_json_object(to_json(col("requestParameters")), "$.policy").alias(
                "policy_json"
            ),
        )
        ...
        exploded_data = flattened_data.select(
            "*",
        ...
        return exploded_data.select(
            "*",
            regexp_replace(col("Resource2_renamed"), "arn:aws:s3:::", "s3://").alias(
                "storage_path"
            ),
        )
    ...
    def all_tables_details():
        ...
        df_tables = dlt.read("system_all_table_details").alias("a")
        df_cloudtrail = dlt.read("cloudtrail_logs_with_storagepath").alias("b")

        # Perform LEFT JOIN
        joined_df = df_tables.join(
            df_cloudtrail, col("a.storage_path") == col("b.storage_path"), how="left"
        )

        # Determine if the access is internal or external based on userAgent
        enriched_df = (
            joined_df.withColumn(
                "is_read", when(col("b.Action2").isin(read_actions), 1).otherwise(0)
            )
            .withColumn(
                "is_write", when(col("b.Action2").isin(write_actions), 1).otherwise(0)
            )
            .withColumn(
        ...
        grouped_df = enriched_df.groupBy(
            ...
            spark_sum("is_read").alias("reads"), spark_sum("is_write").alias("writes")
        )
    ...
    def all_tables_details():
        # Read source datasets
        df_tables = dlt.read("system_all_table_details").withColumn(
        ...
        for col_name in null_columns:
            enriched_df = df_tables.withColumn(col_name, F.lit(None).cast(StringType()))
        grouped_df = enriched_df.groupBy(
            "processed_date",
            "sourced_from_cloudlogs",
            "a.table_catalog",
